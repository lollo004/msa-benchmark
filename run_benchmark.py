...
if __name__ == "__main__":
    ...
    # Runs on 16 threads from parameter
    results_mafft = benchmark_algorithm(
        algorithm_name="mafft",
        command_template="mafft --thread 16 {inputs}/{input_file} > {outputs}/{output_file}"
    )
    save_results(results_mafft, output_csv="csv/mafft_benchmark_results.csv")
    ...
    save_results(results_clustalw, output_csv="csv/clustalw2_benchmark_results.csv")

    # Runs on 16 threads from parameter - Unable to test!
    #results_tcoffee = benchmark_algorithm(
    #    algorithm_name="tcoffee",
    #    command_template="sources/t_coffee/bin/t_coffee -thread 16 -output clustalw -outfile {output_file} -seq {inputs}/{input_file} -outfile {outputs}/{output_file}"
    #)
    #save_results(results_tcoffee, output_csv="csv/tcoffee_benchmark_results.csv")

    # 16 Threads from cli args
    results_pasta = benchmark_algorithm(
        algorithm_name="pasta",
        command_template="python3 sources/pasta/run_pasta.py --auto --num-cpus=16 -i {inputs}/{input_file} -o {outputs} --datatype=protein"
    )
    save_results(results_pasta, output_csv="csv/pasta_benchmark_results.csv")

    # 16 threads from cli args
    results_clustalo = benchmark_algorithm(
        algorithm_name="clustalo",
        command_template="{binaries}/clustalo --threads=16 -o {outputs}/{output_file} -i {inputs}/{input_file} -t Protein"
    )
    save_results(results_clustalo, output_csv="csv/clustalo_benchmark_results.csv")
    
    # 16 threads from cli args
    results_famsa = benchmark_algorithm(
        algorithm_name="famsa",
        command_template="{binaries}/famsa -t 16 {inputs}/{input_file} {outputs}/{output_file}"
    )
    save_results(results_famsa, output_csv="csv/famsa_benchmark_results.csv")    

    # Runs on 16 threads automatically
    results_muscle = benchmark_algorithm(
        algorithm_name="muscle5",
        command_template="{binaries}/muscle5 -align {inputs}/{input_file} -output {outputs}/{output_file}"
    )
    save_results(results_muscle, output_csv="csv/muscle5_benchmark_results.csv")
